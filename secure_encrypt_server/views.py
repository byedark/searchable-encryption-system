...
def remove_file(filename): #移除文件部分
...
def build_searchable_cloud_syetem(request):
  ...
  response = {
    ...
    # secret里的东西最为重要
    "secret":{
      "aes_secret":aes_password_list, #密钥
      "word":word, # 构建查询向量要用
      # "encrypt_matrix":encrypt_matrix,
      # 对矩阵做了一次混淆，需要用逆矩阵还原
      # 这个矩阵的列和行就是上传文件的数量，矩阵的乘法，左矩阵的列要和右矩阵的行一样
      "decrypt_matrix":decrypt_matrix #解密矩阵
    }
  }
