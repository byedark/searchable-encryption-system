...
def build_stop_words():
  ff = open('secure_encrypt_server/stop_words/1.txt', encoding='utf-8')
  data = ff.read()
  ans = data.splitlines()
  ff.close()
  return ans
...
def tf_idf(data_lib):
  ...
  # 计算tf-idf值
  # data_lib = change_stop(data_lib)
  # 词频特征提取
  vectorizer = CountVectorizer(stop_words=build_stop_words())
  ...
  # 必须是一个可逆矩阵才行
  # 矩阵可能不可逆，正好成比例，所以需要判断，循环生成以保证生成的矩阵一定可逆
  while True:
    # 随机加密矩阵
