<template>
  <div class="upload-fun">
    ...
    v-model="giveup_modal">
      <div slot="default">
        <div class="normal-font center">
          <Icon type="md-alert" size="16" class="error"/> 这将删除已经提交的文件，删除后无法恢复是否确认删除？
        </div>
      </div>
      <div slot="footer">
...
export default {
  ...
  data:function(){
    return {
      ...
      max_file_number:200,
      max_file_size:2048,
      //仅仅支持上传如下格式 word，excel等难以通过文本形式显示，以后需要展开下载模块，对文件格式进行判断，使得word文件无法预览但可以提供下载
      fomatter:["txt","c","cpp","py","js","css","html","vue","json","md"],
      //同一批次文档标记，是自动随机生成的
      label:null,
      password:null,
      username:null,
  ...
  methods:{
    ...
    //用before-file的时候默认会有一个file参数，也就是一个文件的参数，是一个自定义的要求
    //如果返回false，上传就会失败-------------------------------------------------------------
    addItem:function(file){
      ...
      // 将文件加入显示列表中
      //既然返回了一个参数file，借此机会赶紧写进data，赋个值，就不用自己传参了
      let item = {
        file:file,//这个file就是before_upload返回回来的值
        name:file.name,//这个名字就是file的文件名，也在返回值里
        uploaded:false,
        display:false,
        error_info:"",
        ex:"",//文件的权限表达式
    ...
    //和additem一起的函数，将功能分离，避免函数过长难以DEBUG--------------------------------------------
    ...
    check_ex:function(ex)
    {
      ...
      if(ex.length <= 0)
         return true;
      //精妙做法：如何判断这个表达式是否合法，直接把字符串忽略就可以对表达式运算了
      // 将&|!()\s之外的符号替换为0
      ex = ex.replace(/[^\s&|!()]+/g,"0");
      // 如果不构成一个表达式则说明格式不对
      //把字符串换成数字这样子就能直接形成一个表达式了，可以换成1或者0都OK
      try {
        eval(ex);
      } catch {
        return "格式错误";
